...
import { CreateOrUpdateTransferInput, GetAllTransferOutput, UpdateTransferInput } from '../transfer/dto/transferDto';

class pointToPointTransferService {
  ...
  public async getAllTransferPackageDetails(requestObj: any,userId:Number,sourceTpId:Number) {
      console.log('From service ',requestObj);
    let result = await http.post(`/Transfer/GetP2PTransferPackageDetails?userId=${userId}&sourceTransferPointId=${sourceTpId}`, requestObj);
    return result.data.result;
  }
  ...
  public async postTransfer(postBody: any) {
    ...
    let result = await http.post('/Transfer/SubmitP2PTransfer', postData, configHeader);
    return result.data.result;
  }

  public async acknowledgeTransfer(transferId: Number){
    let result = await http.get('/Transfer/AcknowledgeP2PTransfer', { params:  { transferId: transferId } }    );
    return result.data.result;
  }
}

export default new pointToPointTransferService;