...
const ReviewPackage = (props: any) => {
    ...
    const [dataSourceCounter, setdataSourceCounter] = useAppState(TransferStore.AppStateKeys.EXCHANGE_COUNTER, 1);
    const [currentExchangeCurrency, setcurrentExchangeCurrency] = useCompState('');
    const [editTransfer] = useAppState(TransferStore.AppStateKeys.EDIT_TRANSFER);
    const [retainTransferHistory, setRetainTransferHistory] = useAppState(TransferStore.AppStateKeys.RETAIN_TRANSFER_DATA);
    const [isPartialTransfer, setIsPartialTransfer] = useAppState(TransferStore.AppStateKeys.IS_PARTIAL_TRANSFER,false);
    ...
    const renderSelectedItems = () => {
        ...
        if (editTransfer != undefined && editTransfer?.items?.length > 0 && !retainTransferHistory) {
            compState.dataList = transferPackageDetails.packageContents.map((item: any) => {
                return {
                    amount: item.transferAmount,
                    currency: transferPackageDetails.currencies.find((cur: any) => cur.symbol == item.currencySymbol)?.symbol
                }
            })
    ...
    return (
        <>
                    ...
                    <Card size="small">
                        ...
                        <CustomRow className="record-view">
                            ...
                            <CustomCol
                                ...
                                sm={{ span: 24 }}
                                xs={{ span: 10 }}
                                className="center-block-alignment">
            ...
            {dataSource && dataSource.length > 0 && <CustomRow className="top-margin-10px row-padding-15px">
                ...
                <Card className="custom-card exchange-payments" title={L('Selected Payments Total')}>
                        ...
                        </CustomRow>
                        {
                            dataSource && compState.currencyList && compState.currencyList.length > 0 && dataSource.length > 0 && dataSource.map((dataRow: any, index: any) => (
                                <CustomDataRepeater key={index}
                                    ...
                                    isPartialTransferAllowed = {true} //HOTFIX: commenting isPartialTransfer, hard coding to ignore partial transfer logic 
                                     />
