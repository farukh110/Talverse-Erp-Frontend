...
const Dispatch = (props: any) => {
    ...
    const { validations, transferPoints, formRef, packageId } = props;
    const [fileListState, setFileListState] = useAppState(TransferStore.AppStateKeys.FILE_LIST, { fileList: [] });
    const [subFormFields, setSubFormFields] = useAppState(TransferStore.AppStateKeys.TRANSFER_DISPATCH_DYNAMIC_DETAILS, []);
    const [destinationListAssociation, setDestinationListAssociation] = useAppState(TransferStore.AppStateKeys.ASSOCIATED_DESTINATION_LIST);
    ...
    const getDestinationList = async () => {
        compState.destinationSelctedValue = formRef?.getFieldValue(TransferStore.AppStateKeys.originOrDestination.SELECT_DESTINATION);
        if (transferPoints && transferPoints.length > 0) {
            setNewTransferForm();
        }
        else if (transferPoints && transferPoints.length == undefined) {
    ...
    const setNewTransferForm = () => {
        try {
            ...
            // populating data items for Destination dropDown
             ...
             // can be refacotred
            let filteredDestinationList: any = [];
            for (let i = 0; i < transferPoints.length; i++) {
                let transferPoint = destinationListAssociation.find((item: any) => item.key == transferPoints[i].id);
                if (transferPoint) {
                    filteredDestinationList.push(transferPoints[i]);
                }
            }
            populateDestinationList(filteredDestinationList, destinationList);
    ...
    const setEditTransferForm = () => {
        try {
            ...
            // can be refacotred
            let filteredDestinationList: any = [];
            for (let i = 0; i < transferPoints.destinationList.length; i++) {
                let transferPoint = destinationListAssociation.find((item: any) => item.key == transferPoints.destinationList[i].id);
                if (transferPoint) {
                    filteredDestinationList.push(transferPoints.destinationList[i]);
                }
            }
            populateDestinationList(filteredDestinationList, destinationList);
            compState.destinationList = destinationList;
    ...
    const showUploadComponent = (dynamicForm: any) => {
        try {
            ...
            if (uploadComp) {
                compState.uploadCompValidation = uploadComp.validations ? formUtils.getFormValidationObject(uploadComp.validations) : [];
                return true;
            }
        } catch (ex) {
            console.log(`error: ${ex}`);
        ...
        <>
            <div className='record-view-padding'>
                ...
                <Badge.Ribbon className='custom-badge' text={`Package Id: ${packageId}`}>
                    ...
                    <CustomRow className="record-view-padding-0">

                        <Card className='border-less-card' bordered={false}>
                            <CustomRow gutter={[40, 16]}>
                                <CustomCol className="form-area" span={12}>
                                    <CustomDropdownInput
                                        dataItems={compState.destinationList}
                                        onChange={handleChange}
                                        label={L(LocalizationKeys.ERP_LblSelectDestination.key)}
                                        name={TransferStore.AppStateKeys.originOrDestination.SELECT_DISPATCH_DESTINATION}
